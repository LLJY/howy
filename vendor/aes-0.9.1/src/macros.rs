...
macro_rules! impl_backends {
    (
        ...
        #[cfg(feature = "zeroize")]
        impl crate::howy_zeroize::Flat for $enc_name {
            fn zeroed() -> Self {
                Self {
                    keys: <$keys_ty as crate::howy_zeroize::Flat>::zeroed(),
                }
            }

            fn wipe(&mut self) {
                crate::howy_zeroize::Flat::wipe(&mut self.keys);
            }

            #[cfg(test)]
            fn is_zero(&self) -> bool {
                crate::howy_zeroize::Flat::is_zero(&self.keys)
            }
        }
        ...
        impl From<$enc_name> for $dec_name {
            ...
            fn from(enc: $enc_name) -> $dec_name {
                #[cfg(feature = "zeroize")]
                {
                    let enc_keys = crate::howy_zeroize::Guard::new_owner(enc.keys);
                    let keys = unsafe { $inv_keys(&enc_keys) };
                    Self { keys }
                }

...
pub(crate) mod aes128 {
    ...
    pub(crate) unsafe fn expand_key(key: &[u8; 16]) -> Aes128RoundKeys {
        ...
        #[cfg(feature = "zeroize")]
        let mut keys = crate::howy_zeroize::Guard::new(zeroed::<Aes128RoundKeys>());
...
pub(crate) mod aes192 {
    ...
    pub(crate) unsafe fn expand_key(key: &[u8; 24]) -> Aes192RoundKeys {
        ...
        #[cfg(feature = "zeroize")]
        let mut keys = crate::howy_zeroize::Guard::new(zeroed::<Aes192RoundKeys>());
        #[cfg(not(feature = "zeroize"))]
        ...
        let (k0, k1l) = {
            #[cfg(feature = "zeroize")]
            let mut t = crate::howy_zeroize::Guard::new([0u8; 32]);
            #[cfg(not(feature = "zeroize"))]
            let mut t = [0u8; 32];
...
pub(crate) mod aes256 {
    ...
    pub(crate) unsafe fn expand_key(key: &[u8; 32]) -> Aes256RoundKeys {
        ...
        #[cfg(feature = "zeroize")]
        let mut keys = crate::howy_zeroize::Guard::new(zeroed::<Aes256RoundKeys>());
        #[cfg(not(feature = "zeroize"))]
...
pub(crate) unsafe fn inv_keys<const N: usize>(keys: &[__m128i; N]) -> [__m128i; N] {
    #[cfg(feature = "zeroize")]
    let mut inv_keys = crate::howy_zeroize::Guard::new(zeroed::<[__m128i; N]>());
    #[cfg(not(feature = "zeroize"))]

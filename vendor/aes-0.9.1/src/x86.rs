...
use crate::howy_zeroize::WipingOnceCell as OnceCell;
#[cfg(all(target_arch = "x86_64", aes_backend = "avx512"))]
...
// Compile-time coverage: selecting either optional VAES source path with
// zeroization requires the corresponding broadcast schedule to be guardable.
#[cfg(all(
    feature = "zeroize",
    target_arch = "x86_64",
    any(aes_backend = "avx256", aes_backend = "avx512")
...
const _: () = {
    ...
    let _ = assert_flat::<Simd256RoundKeys<15>>;
};

#[cfg(all(feature = "zeroize", target_arch = "x86_64", aes_backend = "avx512"))]
const _: () = {
    fn assert_flat<T: crate::howy_zeroize::Flat>() {}
    let _ = assert_flat::<Simd512RoundKeys<15>>;
};
...
mod howy_vaes_zeroize_tests {
    use super::*;

    #[test]
    fn initialized_vaes256_broadcast_schedule_wipes_on_drop() {
        let _test_lock = crate::howy_zeroize::TEST_LOCK.lock().unwrap();
        crate::howy_zeroize::reset_wipe_counts();
        let cell: OnceCell<Simd256RoundKeys<15>> = OnceCell::new();
        cell.get_or_init(|| unsafe { core::mem::transmute([[0xa5u8; 32]; 15]) });
        drop(cell);
    ...
    #[test]
    fn vaes256_broadcast_construction_wipes_on_unwind() {
        if !std::arch::is_x86_feature_detected!("avx2") {
            return;
        }

        let _test_lock = crate::howy_zeroize::TEST_LOCK.lock().unwrap();
        let keys: Simd128RoundKeys<15> = unsafe { core::mem::transmute([[0x5au8; 16]; 15]) };
        crate::howy_zeroize::arm_checkpoint();
        let result = std::panic::catch_unwind(|| unsafe {
            let _ = vaes256::encdec::broadcast_keys(&keys);
        });
    ...
    #[test]
    fn vaes256_is_selected_when_runtime_features_are_available() {
        if std::arch::is_x86_feature_detected!("avx") && std::arch::is_x86_feature_detected!("vaes")
        {
            assert!(matches!(Features::new().dispatch(), Backend::Vaes256));
        }
    }

    #[cfg(aes_backend = "avx512")]
    #[test]
    fn initialized_vaes512_broadcast_schedule_wipes_on_drop() {
        let _test_lock = crate::howy_zeroize::TEST_LOCK.lock().unwrap();
        crate::howy_zeroize::reset_wipe_counts();
        let cell: OnceCell<Simd512RoundKeys<15>> = OnceCell::new();
        cell.get_or_init(|| unsafe { core::mem::transmute([[0xa5u8; 64]; 15]) });
        drop(cell);
        let (events, failures) = crate::howy_zeroize::wipe_counts();
        assert!(events >= 1);
        assert_eq!(failures, 0);
    ...
    #[cfg(aes_backend = "avx512")]
    #[test]
    fn vaes512_broadcast_construction_wipes_on_unwind() {
        ...
        crate::howy_zeroize::arm_checkpoint();
        let result = std::panic::catch_unwind(|| unsafe {
            let _ = vaes512::encdec::broadcast_keys(&keys);
        });
        assert!(result.is_err());
        let (events, failures) = crate::howy_zeroize::wipe_counts();
        assert!(events >= 1);
    ...
    #[cfg(aes_backend = "avx512")]
    #[test]
    fn vaes512_is_selected_when_runtime_features_are_available() {
        if std::arch::is_x86_feature_detected!("avx512f")
            && std::arch::is_x86_feature_detected!("vaes")
        {
            assert!(matches!(Features::new().dispatch(), Backend::Vaes512));
        }
    }
}

...
impl FieldElement {
    ...
    unsafe fn to_uint64x2_t(self) -> uint64x2_t {
        #[cfg(feature = "zeroize")]
        {
            let bytes = crate::howy_zeroize::Guard::new(ByteArray::from(self));
            load_bytes(&bytes)
        }
        #[cfg(not(feature = "zeroize"))]
...
pub(super) unsafe fn expand_key(h: &[u8; 16]) -> ExpandedKey {
    #[cfg(feature = "zeroize")]
    let mut intermediates: crate::howy_zeroize::Guard<[uint64x2_t; 8]> =
        crate::howy_zeroize::Guard::new(core::mem::zeroed());
    #[cfg(not(feature = "zeroize"))]
    let mut intermediates: [uint64x2_t; 8] = core::mem::zeroed();
    ...
    intermediates[H1] = load_bytes(h);
    intermediates[D1] = compute_d(intermediates[H1]);

    // Compute powers using R/F multiplication
    intermediates[H2] = gf128_mul_rf(intermediates[H1], intermediates[H1], intermediates[D1]);
    intermediates[D2] = compute_d(intermediates[H2]);

    intermediates[H3] = gf128_mul_rf(intermediates[H2], intermediates[H1], intermediates[D1]);
    intermediates[D3] = compute_d(intermediates[H3]);

    intermediates[H4] = gf128_mul_rf(intermediates[H2], intermediates[H2], intermediates[D2]);
    intermediates[D4] = compute_d(intermediates[H4]);

    #[cfg(feature = "zeroize")]
    {
        let mut expanded = crate::howy_zeroize::Guard::new(ExpandedKey::default());
        expanded.h1 = FieldElement::from_uint64x2_t(intermediates[H1]);
        expanded.d1 = FieldElement::from_uint64x2_t(intermediates[D1]);
        expanded.h2 = FieldElement::from_uint64x2_t(intermediates[H2]);
        expanded.d2 = FieldElement::from_uint64x2_t(intermediates[D2]);
        expanded.h3 = FieldElement::from_uint64x2_t(intermediates[H3]);
        expanded.d3 = FieldElement::from_uint64x2_t(intermediates[D3]);
        expanded.h4 = FieldElement::from_uint64x2_t(intermediates[H4]);
        expanded.d4 = FieldElement::from_uint64x2_t(intermediates[D4]);
        crate::howy_zeroize::checkpoint();
        expanded.take()
    }
    #[cfg(not(feature = "zeroize"))]
    ExpandedKey {
        h1: FieldElement::from_uint64x2_t(intermediates[H1]),
        d1: FieldElement::from_uint64x2_t(intermediates[D1]),
        h2: FieldElement::from_uint64x2_t(intermediates[H2]),
        d2: FieldElement::from_uint64x2_t(intermediates[D2]),
        h3: FieldElement::from_uint64x2_t(intermediates[H3]),
        d3: FieldElement::from_uint64x2_t(intermediates[D3]),
        h4: FieldElement::from_uint64x2_t(intermediates[H4]),
        d4: FieldElement::from_uint64x2_t(intermediates[D4]),
    }
}

// Compile-time coverage: selecting the AArch64 intrinsic source path with
// zeroization requires its complete SIMD construction array to be guardable.
#[cfg(feature = "zeroize")]
const _: () = {
    fn assert_wipe<T: crate::howy_zeroize::Wipe>() {}
    let _ = assert_wipe::<[uint64x2_t; 8]>;
};

...
mod howy_zeroize {
    ...
    {
        fn drop(&mut self) {
            let bytes: &mut [u8] = self.0.as_mut();
            bytes.zeroize();
            #[cfg(test)]
            record_wipe(self.0.as_ref().iter().all(|byte| *byte == 0));
        }
    }
    ...
    #[cfg(test)]
    std::thread_local! {
        static PANIC_POINT: core::cell::Cell<u8> = const { core::cell::Cell::new(0) };
    }
    #[cfg(test)]
    ...
    #[inline]
    pub(crate) fn checkpoint(point: u8) {
        #[cfg(not(test))]
        let _ = point;
        #[cfg(test)]
        if PANIC_POINT.with(|armed| {
            let matches = armed.get() == point;
            if matches {
                armed.set(0);
            }
            matches
        }) {
            panic!("HOWY AES-GCM scratch checkpoint {point}");
        }
    }
    ...
    #[cfg(test)]
    pub(crate) fn arm_checkpoint(point: u8) {
        WIPE_EVENTS.store(0, Ordering::SeqCst);
        WIPE_FAILURES.store(0, Ordering::SeqCst);
        PANIC_POINT.with(|armed| armed.set(point));
    }
    ...
    pub(crate) fn reset_counts() {
        WIPE_EVENTS.store(0, Ordering::SeqCst);
        WIPE_FAILURES.store(0, Ordering::SeqCst);
        PANIC_POINT.with(|armed| armed.set(0));
    }
    ...
    TagSize: crate::TagSize,
{
    fn from(cipher: Aes) -> Self {
        #[cfg(feature = "zeroize")]
        let mut ghash_key = howy_zeroize::Guard::new(ghash::Key::default());
    ...
    TagSize: crate::TagSize,
{
    fn encrypt_inout_detached(
        ...
        #[cfg(feature = "zeroize")]
        let full_tag_bytes = (*full_tag).as_slice();
        #[cfg(not(feature = "zeroize"))]
        let full_tag_bytes = full_tag.as_slice();
        full_tag_bytes[..TagSize::to_usize()]
            .try_into()
            .map_err(|_| Error)
    }
    ...
    fn decrypt_inout_detached(
        ...
        #[cfg(feature = "zeroize")]
        let expected_tag_bytes = (*expected_tag).as_slice();
        #[cfg(not(feature = "zeroize"))]
        let expected_tag_bytes = expected_tag.as_slice();
        if expected_tag_bytes[..TagSize::to_usize()].ct_eq(tag).into() {
    ...
    TagSize: crate::TagSize,
{
    ...
    #[cfg(feature = "zeroize")]
    fn init_ctr(&self, nonce: &Nonce<NonceSize>) -> (Ctr32BE<&Aes>, howy_zeroize::Guard<Block>) {
        ...
        let mut tag_mask = howy_zeroize::Guard::new(Block::default());
        ctr.write_keystream_block(&mut *tag_mask);
        howy_zeroize::checkpoint(2);
        (ctr, tag_mask)
    }
    ...
    fn compute_tag(
        &self,
        mask: howy_zeroize::Guard<Block>,
        associated_data: &[u8],
        buffer: &[u8],
    ) -> howy_zeroize::Guard<Tag> {
        let mut ghash = self.ghash.clone();
        ...
        let mut tag = howy_zeroize::Guard::new(ghash.finalize());
...
#[cfg(all(test, feature = "aes", feature = "alloc", feature = "zeroize"))]
mod howy_tests {
    use super::*;
    use std::vec::Vec;

    #[test]
    fn key_and_tag_mask_guards_wipe_on_error_and_unwind() {
        howy_zeroize::arm_checkpoint(1);
        let key_setup = std::panic::catch_unwind(|| {
            let _ = Aes256Gcm::new(&Key::<Aes256Gcm>::from([0x5a; 32]));
        });
        assert!(key_setup.is_err());
        let (events, failures) = howy_zeroize::wipe_counts();
        assert!(events >= 1);
        assert_eq!(failures, 0);

        let cipher = Aes256Gcm::new(&Key::<Aes256Gcm>::from([0xa5; 32]));
        let nonce = Nonce::<U12>::from([0x3c; 12]);
        howy_zeroize::arm_checkpoint(2);
        let tag_mask = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let mut buffer = Vec::from(&b"guarded plaintext"[..]);
            let _ = cipher.encrypt_in_place(&nonce, b"aad", &mut buffer);
        }));
        assert!(tag_mask.is_err());
        let (events, failures) = howy_zeroize::wipe_counts();
        assert!(events >= 1);
        assert_eq!(failures, 0);

        let mut combined = Vec::from(&b"guarded plaintext"[..]);
        cipher
            .encrypt_in_place(&nonce, b"aad", &mut combined)
            .unwrap();
        let last = combined.last_mut().unwrap();
        *last ^= 1;
        howy_zeroize::reset_counts();
        assert!(
            cipher
                .decrypt_in_place(&nonce, b"aad", &mut combined)
                .is_err()
        );
        let (events, failures) = howy_zeroize::wipe_counts();
        assert!(events >= 2, "tag mask and expected tag must both wipe");
        assert_eq!(failures, 0);
    }
}

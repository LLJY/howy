...
mod tests {
    ...
    #[test]
    fn pam_silent_prompt_off_uses_fresh_authenticate_v1_connection_without_conversation() {
        reset_allocations();
        let username = CString::new("alice").unwrap();
        let service = CString::new("sudo").unwrap();
        ...
        let first = std::thread::spawn(move || {
            ...
            let Some(Cmd::BeginAuthV1(begin)) = request.cmd else {
                panic!("silent PAM must still begin with BeginAuthV1")
            };
            assert_eq!(begin.username, "alice");
            assert_eq!(begin.client_nonce, [0x11; PROMPT_NONCE_BYTES]);
            let policy = begin.policy.unwrap();
        ...
        let second = std::thread::spawn(move || {
            ...
            send_message(&mut second_daemon, &Response::success(0, "desk", 0.9, 1.0)).unwrap();
        });
        assert_eq!(
            run_adaptive_auth(&mut items, vec![first_client, second_client], PAM_SILENT),
            PAM_SUCCESS
        );
        first.join().unwrap();
        second.join().unwrap();
        assert_eq!(items.user_calls.get(), 1);
        assert_eq!(items.item_calls.get(), 2);
        assert_eq!(conversation_state.calls, 0);
        assert_cleanup(0, 0);
    }

    #[test]
    fn pam_silent_prompt_required_cancels_without_getting_or_invoking_conversation() {
        reset_allocations();
        let username = CString::new("alice").unwrap();
        let service = CString::new("sudo").unwrap();
        let mut conversation_state = MockConversation::bytes(b"OK");
        ...
        assert_eq!(
            run_adaptive_auth(&mut items, vec![client], PAM_SILENT),
            PAM_AUTHINFO_UNAVAIL
        );
        daemon_thread.join().unwrap();
        assert_eq!(items.user_calls.get(), 1);
